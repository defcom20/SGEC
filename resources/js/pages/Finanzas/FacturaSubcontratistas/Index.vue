...
const onSearch = useDebounceFn((value) => {
    router.get('/factura-subcontratistas', { search: value }, {
        preserveState: true,
        replace: true,
    })
}, 300)
...
const onDeleteConfirm = () => {
    if (itemToDelete.value) {
        router.delete(`/factura-subcontratistas/${itemToDelete.value.uuid}`, {
            onSuccess: () => {
...
const modalConfig = {
    ...
    message: 'Esta acción eliminará el registro de la factura del subcontratista.',
...
const breadcrumbs = [
    ...
    { title: 'Facturas Subcontratistas', href: '#' },
]
...
const getStatusBadge = (status) => {
    const styles = {
        ...
        'pagado_completo': 'bg-green-100 text-green-800 border-green-200',
    }
    const labels = {
        'pendiente': 'Pendiente',
    ...
    return { class: styles[status] || 'bg-gray-100', label: labels[status] || status }
}
</script>

<template>
    <Head title="Facturas de Subcontratistas" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="page-container">
            <div class="header">
                <div>
                    <h1>Facturas Subcontratistas</h1>
                    <p class="subtitle">Gestión de cuentas por pagar</p>
                </div>
                <Link href="/factura-subcontratistas/create" class="btn btn-primary">
                    ...
                    <tbody>
                        <tr v-for="factura in facturas.data" :key="factura.id">
                            <td class="font-medium">
                                <div class="doc-info">
                                    <span class="doc-type">{{ factura.tipo_documento }}</span>
                                    <span>{{ factura.serie }}-{{ factura.numero_documento }}</span>
                                </div>
                            </td>
                            <td>{{ formatDate(factura.fecha_emision) }}</td>
                            <td>
                                <div class="client-info">
                                    <span class="client-name">{{ factura.subcontratista?.razon_social_nombre }}</span>
                                    <span class="sub-text">RUC: {{ factura.subcontratista?.numero_documento }}</span>
                                </div>
                            </td>
                            <td>
                                <span v-if="factura.orden_servicio" class="ref-badge">
                                    OS: {{ factura.orden_servicio.numero_orden }}
                                </span>
                                <span v-else class="text-muted text-xs">-</span>
                            </td>
                            <td class="text-right font-medium">{{ formatCurrency(factura.total) }}</td>
                            <td class="text-right font-medium text-danger">{{ formatCurrency(factura.monto_pendiente) }}</td>
                            <td class="text-center">
                                <span :class="['status-badge', getStatusBadge(factura.estado).class]">
                                    {{ getStatusBadge(factura.estado).label }}
                                </span>
                            </td>
                            <td class="text-right actions-cell">
                                <Link :href="`/factura-subcontratistas/${factura.uuid}/edit`" class="action-btn edit">
...
.doc-type { font-size: 0.65rem; color: var(--muted-foreground); text-transform: uppercase; }

.client-info { display: flex; flex-direction: column; }
.client-name { font-weight: 500; }
.sub-text { color: var(--muted-foreground); font-size: 0.65rem; }

.ref-badge { background: var(--muted); padding: 0.1rem 0.4rem; border-radius: 4px; font-size: 0.7rem; white-space: nowrap; }

.status-badge { padding: 0.15rem 0.5rem; border-radius: 999px; font-size: 0.65rem; font-weight: 600; border: 1px solid transparent; display: inline-block; }
.status-badge.bg-yellow-100 { background: #fef9c3; color: #854d0e; border-color: #fde047; }
.status-badge.bg-blue-100 { background: #dbeafe; color: #1e40af; border-color: #bfdbfe; }
.status-badge.bg-green-100 { background: #dcfce7; color: #166534; border-color: #bbf7d0; }
